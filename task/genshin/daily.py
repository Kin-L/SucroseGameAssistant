...
class Daily(Genshin):
    ...
    def check_resin(self):
        ...
        self.open_sub("地图")
        wait(800)
        ori = int((ocr((1101,26, 1213,63))[0].split("/")[0]).strip(" "))
        self.home()
        #检查浓缩树脂
        self.open_sub("背包")
        wait(2000)
    ...
    def genshin_make_condensed(self,ori,num):
        ...
        click((1000, 500))
        wait(1000)
        if "合成" in ocr((126,31,217,68))[0]:
        #识别浓缩树脂是否能做
            if "浓缩树脂" in ocr((1258,102,1417,148))[0]:
    ...
    def daily_gift(self):
        ...
        wait(800)
        x, y = find_pic(r"assets\genshin\picture\condensed\daily_task_gift.png",(1502,696,1591,804))[0]
        if (x, y) != (0, 0):
            click((1552,753)) #完成每日任务
            wait(1000)
            click((1552,753))
            wait(1000)
            self.indicate("每日任务已完成，准备前往领取每日奖励")
        x, y = find_pic(r"assets\genshin\picture\condensed\daily_task_done.png",(757,696,1145,803))[0]
        if (x, y) != (0, 0) and "领取奖励" in ocr((447,812,677,890))[0]:
            self.home()
            self.tp_fontaine1()
        ...
        elif (x, y) != (0, 0) and "今日奖励已领取" in ocr((447,812,677,890))[0]:
            self.indicate("今日奖励已领取")
        else:
            self.indicate("今日奖励领取未知错误")
