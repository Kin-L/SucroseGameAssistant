...
1.  从网盘下载带"full"后缀的最新安装包。蓝奏云有最大包体限制，需要额外下载OCR组件（大多数电脑使用“PaddleOCR-json”），请参考快速上手教程放置OCR组件  
    1.  SGA-full包体  
...
## 命令行与参数
1.  使用SGA-c.exe启动，允许SGA启动时自带命令行窗口，该窗口只显示SGA实时日志信息，没有其他命令行参数可用于控制SGA。可通过UI界面上的按钮隐藏该窗口。  
2.  SGA支持参数启动，参数启动方式如下：  
#### current 参数
启动后自动执行当前页面的任务，相当于启动SGA后直接点击开始按钮。例：  
```bash
SGA.exe -current
```

#### subconfig 参数
启动后自动执行指定的子任务，后面需要追加子任务设置信息的识别码(四位数字的编号)，可在"personal\config"文件夹下查看对应关系。  
例：在"personal\config"文件夹下有一个配置文件"9815默认配置1.json"，"默认配置1"为自拟名，"9815"为识别码，则启动命令为：
```bash
SGA.exe -subconfig 9815
```
#### hideui 参数
启动后隐藏SGA界面，使用该参数启动将无法对SGA进行任何操作，关闭cmd窗口可结束SGA。  
该模式配合SGA-c.exe和current/subconfig参数使用，或启动后在后台等待定时执行。例如：
```bash
SGA-c.exe -hideui
```
#### back 参数
使用该参数时，启动SGA后不激活界面窗口；否则SGA将在启动后尝试将窗口激活到前台一次。  
例如：
```bash
SGA.exe -back
```
